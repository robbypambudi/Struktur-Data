...
//
void find(AVLNode *node, int target, bool *found, int *sum)
{
    if (node)
    {
        if (!(*found) && *sum == 0)
        {
            if (target < node->data)
            {
                find(node->left, target, found, sum);
            }
        }
        if (!(*found) && *sum == 0)
        {
            if (target > node->data)
            {
                find(node->right, target, found, sum);
            }
        }
    ...
    while (test--)
    {
        ...
        else
        {
            if (avl_find(&avlku, value))
            {
                int sum = 0;
                bool isFound = false;
                find(avlku._root, value, &isFound, &sum);
                printf("%d\n", sum);
            }
            else
